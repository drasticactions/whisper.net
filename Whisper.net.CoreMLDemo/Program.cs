...

{
    ...
    var realPath = Path.GetDirectoryName(typeof(NativeLibraryLoader).Assembly.Location);
    if (!Directory.Exists(Path.Combine(realPath, Path.GetFileNameWithoutExtension(opt.ModelName) + "-encoder.mlmodelc")))
    {
        throw new FileNotFoundException($"{Path.GetFileNameWithoutExtension(opt.ModelName) + "-encoder.mlmodelc"} is missing from the current directory." +
                                        $"This is required in order to run the demo with CoreML. You can build it via the python scripts in whisper.cpp, https://github.com/ggerganov/whisper.cpp#core-ml-support");
    }
    #endif
